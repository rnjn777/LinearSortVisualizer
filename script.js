document.addEventListener('DOMContentLoaded', () => {
    ...
    async function sleep() {
        ...
        if (isStepMode) {
            const originalText = statusBarText.innerHTML;
            updateStatus(originalText + "<br><b>-- Paused: Click 'Next Step' to advance --</b>");
            await new Promise(resolve => {
                pauseResolver = resolve;
            });
            updateStatus(originalText);
    ...
    function setControls(enabled) {
        ...
        if (enabled) { // Stopping or resetting
            isPaused = false;
            isStepMode = false;
    ...
    function playMusic() {
        if (!musicStarted) {
            bgMusic.play().catch(e => console.error("Audio play failed. User must interact again.", e));
            musicStarted = true;
        }
    }
    ...
    muteBtn.addEventListener('click', () => {
        ...
        if (bgMusic.muted) {
            ...
            muteBtn.classList.remove('btn-pause-active');
        } else {
